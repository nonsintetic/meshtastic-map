...
// Function to set up API routes on an Express app
function setupApiRoutes(app, prisma) {
  // load protobufs
  const root = new protobufjs.Root();
  root.resolvePath = (origin, target) =>
    path.join(__dirname, "../protos", target);
  ...
  function formatNodeInfo(node) {
    return {
      ...
      hardware_model_name:
        HardwareModel.valuesById[node.hardware_model] ?? null,
    ...
    const links = [
      ...
      },
      {
        ...
        params: {
          ...
          time_from:
            "Only include metrics created after this unix timestamp (milliseconds)",
          time_to:
            "Only include metrics created before this unix timestamp (milliseconds)",
        },
      },
      {
      ...
      },
      {
        ...
        params: {
          time_from:
            "Only include positions created after this unix timestamp (milliseconds)",
          time_to:
            "Only include positions created before this unix timestamp (milliseconds)",
        },
      },
      {
      ...
      },
      {
        ...
        params: {
          ...
          last_id:
            "Only include messages before or after this id, based on results order",
          count: "How many results to return",
  ...
  app.get("/api/v1/nodes", async (req, res) => {
    try {
      ...
      const hardwareModel = req.query.hardware_model
        ? parseInt(req.query.hardware_model)
        : undefined;
