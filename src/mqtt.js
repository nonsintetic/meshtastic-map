...
const optionsList = [
  ...
  },
  {
    ...
    typeLabel: "<portnum> ...",
    description:
      "If provided, only packets with these portnums will be processed.",
  },
  {
    name: "log-unknown-portnums",
    type: Boolean,
    description:
      "This option will log packets for unknown portnums to the console.",
  },
  {
  ...
  },
  {
    ...
    type: Boolean,
    description:
      "This option will save all received positions to the database.",
  },
  {
  ...
  },
  {
    ...
    type: Boolean,
    description:
      "This option will prevent saving direct messages to the database.",
  },
  {
  ...
  },
  {
    ...
    type: Boolean,
    description:
      "This option will drop all packets that have 'OK to MQTT' set to false.",
  },
  {
    ...
    typeLabel: "<portnum> ...",
    description:
      "If provided, packets with these portnums will be dropped if they don't have a bitfield. (bitfield available from firmware v2.5+)",
  },
  {
    name: "old-firmware-position-precision",
    type: Number,
    description:
      "If provided, position packets from firmware v2.4 and older will be truncated to this many decimal places.",
  },
  {
    name: "forget-outdated-node-positions-after-seconds",
    type: Number,
    description:
      "If provided, nodes that haven't sent a position report in this time will have their current position cleared.",
  },
  {
  ...
  },
  {
    ...
    type: Number,
    description:
      "Map reports older than this many seconds will be purged from the database.",
  },
  {
    name: "purge-neighbour-infos-after-seconds",
    type: Number,
    description:
      "Neighbour infos older than this many seconds will be purged from the database.",
  },
  {
  ...
  },
  {
    ...
    type: Number,
    description:
      "Service envelopes older than this many seconds will be purged from the database.",
  },
  {
    name: "purge-text-messages-after-seconds",
    type: Number,
    description:
      "Text Messages older than this many seconds will be purged from the database.",
  },
  {
    name: "purge-traceroutes-after-seconds",
    type: Number,
    description:
      "Traceroutes older than this many seconds will be purged from the database.",
  },
  {
    name: "purge-waypoints-after-seconds",
    type: Number,
    description:
      "Waypoints older than this many seconds will be purged from the database.",
  },
];
...
// get options and fallback to environment variables, then default values
// command line args take precedence over env vars, which take precedence over defaults
const mqttBrokerUrl =
  options["mqtt-broker-url"] ??
  process.env.MQTT_BROKER_URL ??
  "mqtt://mqtt.meshtastic.org";
const mqttUsername =
  options["mqtt-username"] ?? process.env.MQTT_USERNAME ?? "meshdev";
const mqttPassword =
  options["mqtt-password"] ?? process.env.MQTT_PASSWORD ?? "large4cats";
const mqttClientId =
  options["mqtt-client-id"] ?? process.env.MQTT_CLIENT_ID ?? null;
const mqttTopics =
  options["mqtt-topic"] ??
  (process.env.MQTT_TOPIC ? [process.env.MQTT_TOPIC] : ["msh/#"]);
const allowedPortnums =
  options["allowed-portnums"] ??
  (process.env.ALLOWED_PORTNUMS
    ? process.env.ALLOWED_PORTNUMS.split(",").map(Number)
    : null);
const logUnknownPortnums =
  options["log-unknown-portnums"] ??
  process.env.LOG_UNKNOWN_PORTNUMS === "true" ??
  false;
const collectServiceEnvelopes =
  options["collect-service-envelopes"] ??
  process.env.COLLECT_SERVICE_ENVELOPES === "true" ??
  false;
const collectPositions =
  options["collect-positions"] ??
  process.env.COLLECT_POSITIONS === "true" ??
  false;
const collectTextMessages =
  options["collect-text-messages"] ??
  process.env.COLLECT_TEXT_MESSAGES === "true" ??
  false;
const ignoreDirectMessages =
  options["ignore-direct-messages"] ??
  process.env.IGNORE_DIRECT_MESSAGES === "true" ??
  false;
const collectWaypoints =
  options["collect-waypoints"] ??
  process.env.COLLECT_WAYPOINTS === "true" ??
  false;
const collectNeighbourInfo =
  options["collect-neighbour-info"] ??
  process.env.COLLECT_NEIGHBOUR_INFO === "true" ??
  false;
const collectMapReports =
  options["collect-map-reports"] ??
  process.env.COLLECT_MAP_REPORTS === "true" ??
  false;
const decryptionKeys =
  options["decryption-keys"] ??
  (process.env.DECRYPTION_KEYS
    ? process.env.DECRYPTION_KEYS.split(",")
    : [
        "1PG7OiApB1nwvP+rz05pAQ==", // add default "AQ==" decryption key
      ]);
const dropPacketsNotOkToMqtt =
  options["drop-packets-not-ok-to-mqtt"] ??
  process.env.DROP_PACKETS_NOT_OK_TO_MQTT === "true" ??
  false;
const dropPortnumsWithoutBitfield =
  options["drop-portnums-without-bitfield"] ??
  (process.env.DROP_PORTNUMS_WITHOUT_BITFIELD
    ? process.env.DROP_PORTNUMS_WITHOUT_BITFIELD.split(",").map(Number)
    : null);
const oldFirmwarePositionPrecision =
  options["old-firmware-position-precision"] ??
  (process.env.OLD_FIRMWARE_POSITION_PRECISION
    ? parseInt(process.env.OLD_FIRMWARE_POSITION_PRECISION)
    : null);
const forgetOutdatedNodePositionsAfterSeconds =
  options["forget-outdated-node-positions-after-seconds"] ??
  (process.env.FORGET_OUTDATED_NODE_POSITIONS_AFTER_SECONDS
    ? parseInt(process.env.FORGET_OUTDATED_NODE_POSITIONS_AFTER_SECONDS)
    : null);
const purgeIntervalSeconds =
  options["purge-interval-seconds"] ??
  (process.env.PURGE_INTERVAL_SECONDS
    ? parseInt(process.env.PURGE_INTERVAL_SECONDS)
    : 10);
const purgeNodesUnheardForSeconds =
  options["purge-nodes-unheard-for-seconds"] ??
  (process.env.PURGE_NODES_UNHEARD_FOR_SECONDS
    ? parseInt(process.env.PURGE_NODES_UNHEARD_FOR_SECONDS)
    : null);
const purgeDeviceMetricsAfterSeconds =
  options["purge-device-metrics-after-seconds"] ??
  (process.env.PURGE_DEVICE_METRICS_AFTER_SECONDS
    ? parseInt(process.env.PURGE_DEVICE_METRICS_AFTER_SECONDS)
    : null);
const purgeEnvironmentMetricsAfterSeconds =
  options["purge-environment-metrics-after-seconds"] ??
  (process.env.PURGE_ENVIRONMENT_METRICS_AFTER_SECONDS
    ? parseInt(process.env.PURGE_ENVIRONMENT_METRICS_AFTER_SECONDS)
    : null);
const purgeMapReportsAfterSeconds =
  options["purge-map-reports-after-seconds"] ??
  (process.env.PURGE_MAP_REPORTS_AFTER_SECONDS
    ? parseInt(process.env.PURGE_MAP_REPORTS_AFTER_SECONDS)
    : null);
const purgeNeighbourInfosAfterSeconds =
  options["purge-neighbour-infos-after-seconds"] ??
  (process.env.PURGE_NEIGHBOUR_INFOS_AFTER_SECONDS
    ? parseInt(process.env.PURGE_NEIGHBOUR_INFOS_AFTER_SECONDS)
    : null);
const purgePowerMetricsAfterSeconds =
  options["purge-power-metrics-after-seconds"] ??
  (process.env.PURGE_POWER_METRICS_AFTER_SECONDS
    ? parseInt(process.env.PURGE_POWER_METRICS_AFTER_SECONDS)
    : null);
const purgePositionsAfterSeconds =
  options["purge-positions-after-seconds"] ??
  (process.env.PURGE_POSITIONS_AFTER_SECONDS
    ? parseInt(process.env.PURGE_POSITIONS_AFTER_SECONDS)
    : null);
const purgeServiceEnvelopesAfterSeconds =
  options["purge-service-envelopes-after-seconds"] ??
  (process.env.PURGE_SERVICE_ENVELOPES_AFTER_SECONDS
    ? parseInt(process.env.PURGE_SERVICE_ENVELOPES_AFTER_SECONDS)
    : null);
const purgeTextMessagesAfterSeconds =
  options["purge-text-messages-after-seconds"] ??
  (process.env.PURGE_TEXT_MESSAGES_AFTER_SECONDS
    ? parseInt(process.env.PURGE_TEXT_MESSAGES_AFTER_SECONDS)
    : null);
const purgeTraceroutesAfterSeconds =
  options["purge-traceroutes-after-seconds"] ??
  (process.env.PURGE_TRACEROUTES_AFTER_SECONDS
    ? parseInt(process.env.PURGE_TRACEROUTES_AFTER_SECONDS)
    : null);
const purgeWaypointsAfterSeconds =
  options["purge-waypoints-after-seconds"] ??
  (process.env.PURGE_WAYPOINTS_AFTER_SECONDS
    ? parseInt(process.env.PURGE_WAYPOINTS_AFTER_SECONDS)
    : null);
    ...
    await prisma.node.updateMany({
      where: {
        position_updated_at: {
          ...
          lt: new Date(
            Date.now() - forgetOutdatedNodePositionsAfterSeconds * 1000
          ),
        },
    ...
    if (envelope.packet.decoded != null) {
      ...
      if (bitfield == null) {
        ...
        if (
          ...
          dropPortnumsWithoutBitfield.includes(portnum)
        ) {
          return;
        }
      }
    ...
    if (portnum === 1) {
      ...
      if (position.latitudeI != null && position.longitudeI) {
        ...
        if (bitfield == null && oldFirmwarePositionPrecision != null) {
          ...
          position.latitudeI = PositionUtil.setPositionPrecision(
            position.latitudeI,
            oldFirmwarePositionPrecision
          );
          position.longitudeI = PositionUtil.setPositionPrecision(
            position.longitudeI,
            oldFirmwarePositionPrecision
          );
      ...
      if (telemetry.deviceMetrics) {
        ...
        data.air_util_tx =
          telemetry.deviceMetrics.airUtilTx !== 0
            ...
            : null;
        data.uptime_seconds =
          telemetry.deviceMetrics.uptimeSeconds !== 0
            ? telemetry.deviceMetrics.uptimeSeconds
            : null;
      ...
      if (telemetry.environmentMetrics) {
        // get metric values
        const temperature =
          telemetry.environmentMetrics.temperature !== 0
            ? telemetry.environmentMetrics.temperature
            : null;
        const relativeHumidity =
          telemetry.environmentMetrics.relativeHumidity !== 0
            ? telemetry.environmentMetrics.relativeHumidity
            : null;
        const barometricPressure =
          telemetry.environmentMetrics.barometricPressure !== 0
            ? telemetry.environmentMetrics.barometricPressure
            : null;
        const gasResistance =
          telemetry.environmentMetrics.gasResistance !== 0
            ? telemetry.environmentMetrics.gasResistance
            : null;
        const voltage =
          telemetry.environmentMetrics.voltage !== 0
            ? telemetry.environmentMetrics.voltage
            : null;
        const current =
          telemetry.environmentMetrics.current !== 0
            ? telemetry.environmentMetrics.current
            : null;
        const iaq =
          telemetry.environmentMetrics.iaq !== 0
        ...
        try {
          // find an existing metric with duplicate information created in the last 15 seconds
          const existingDuplicateEnvironmentMetric =
            await prisma.environmentMetric.findFirst({
              where: {
                node_id: envelope.packet.from,
                packet_id: envelope.packet.id,
                created_at: {
      ...
      if (telemetry.powerMetrics) {
        // get metric values
        const ch1Voltage =
          telemetry.powerMetrics.ch1Voltage !== 0
            ? telemetry.powerMetrics.ch1Voltage
            : null;
        const ch1Current =
          telemetry.powerMetrics.ch1Current !== 0
            ? telemetry.powerMetrics.ch1Current
            : null;
        const ch2Voltage =
          telemetry.powerMetrics.ch2Voltage !== 0
            ? telemetry.powerMetrics.ch2Voltage
            : null;
        const ch2Current =
          telemetry.powerMetrics.ch2Current !== 0
            ? telemetry.powerMetrics.ch2Current
            : null;
        const ch3Voltage =
          telemetry.powerMetrics.ch3Voltage !== 0
            ? telemetry.powerMetrics.ch3Voltage
            : null;
        const ch3Current =
          telemetry.powerMetrics.ch3Current !== 0
        ...
        try {
          // find an existing metric with duplicate information created in the last 15 seconds
          const existingDuplicatePowerMetric =
            await prisma.powerMetric.findFirst({
              where: {
                node_id: envelope.packet.from,
                packet_id: envelope.packet.id,
                created_at: {
